...
export const appStateToUniformFactory = function(){
	const result = new StateToUniformMap([
		...
		new StateFactoryTransform("camera transform", "camera", "u_camera", -10.0, 10.0, 0.1),
		...
		new StateFactoryTransform("model transform", "model", "u_model", -10.0, 10.0, 0.1),
		new StateFactoryFloatArray(
			"model param", "model", "u_modelScale", 
			[
				new StateFactoryFloatItem("scale", "Scale", 0.01, 0, 100.0, 0.01),
			]
		),
	]);
	return result;
...
vec3 getAtomPosition(){
	vec3 pos = u_modelPos + (((u_modelRight * a_position.x) + (u_modelUp * a_position.y) + (u_modelAt * a_position.z)) * u_modelScale);
	return pos;
}
...
void main() {
	...
	//float screenZ = (cameraSpaceLength - u_cameraNearFarFarminusnear.x) / (u_cameraNearFarFarminusnear.z);
	float screenZ = cameraSpaceLength / u_cameraFovhFovvFar.z;
	//float screenZ = (posZRaw * 2.0) - 1.0;
...
const filterCallbackFactory = function(in_step){
	return function(in_sideLength, in_x, in_y, in_z){
		if (((0 === (in_y % in_step)) || (in_y === in_sideLength)) &&
			((0 === (in_z % in_step)) || (in_z === in_sideLength))){
				return true;
		}

		if (((0 === (in_x % in_step)) || (in_x === in_sideLength)) &&
			((0 === (in_z % in_step)) || (in_z === in_sideLength))){
				return true;
		}

		if (((0 === (in_x % in_step)) || (in_x === in_sideLength)) &&
			((0 === (in_y % in_step)) || (in_y === in_sideLength))){
				return true;
		}
